<template>
  ...
  >
    <path
      ...
      :fill="modelValue.liked ? '#EF7D31' : '#8a8a8a'"
      p-id="6796"
    ></path>
  </svg>
</template>
...
const liked = computed(() => props.modelValue.liked);
const unliked = computed(() => props.modelValue.unliked);

const toggleLiked = () => {
  const newLiked = !liked.value;
  const newUnliked = newLiked ? false : unliked.value;

  emit('update:modelValue', { liked: newLiked, unliked: newUnliked });
};
</script>
